...
uintf Game::get_to_play() const {
    return to_play;
}

Result Game::get_result() const {
    return result;
}
...
std::ostream& operator<<(std::ostream& os, const Game &game) {
    ...
    for (uintf i = 0; i < 4; ++i) {
        for (uint j = 0; j < 4; ++j) {
            if (game.board[i * 12 + j * 3 + 0]) os << 'B';
            else os << ' ';
            if (game.board[i * 12 + j * 3 + 1]) os << 'C';
            else os << ' ';
            if (game.board[i * 12 + j * 3 + 2]) os << 'A';
            else os << ' ';
            if (game.frozen[i * 4 + j]) os << '#';
            else os << ' ';
            if (j < 3) os << '|';
        }
        if (i < 3) os << "\n-------------------\n";
...
intf Game::get_top(uintf row, uintf col) const {
    if (board[row * 12 + col * 3 + 2]) return 2;
    if (board[row * 12 + col * 3 + 1]) return 1;
    if (board[row * 12 + col * 3 + 0]) return 0;
    // Empty
    return -1;
}
...
void Game::apply_line(uintf line) {
...
bool Game::can_place(uintf ptype, uintf row, uintf col) const {
    ...
    if (frozen[row * 4 + col]) return false;
...
bool Game::can_move(uintf row1, uintf col1, uintf row2, uintf col2) const {
    ...
    // Frozen spaces
    if (frozen[row1 * 4 + col1] || frozen[row2 * 4 + col2]) return false;
