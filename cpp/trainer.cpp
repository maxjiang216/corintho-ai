...
Trainer::Trainer(uintf num_games, uintf num_logged, uintf num_iterations,
                 ...
                 uintf random_seed)
    : num_games{std::max(num_games, (uintf)1)},
      num_iterations{std::max(num_iterations, (uintf)2)}, iterations_done{0},
      games_done{0}, is_done{vector<bool>(num_games, false)},
...
uintf Trainer::place_next(uintf seed, const Game &game, uintf depth,
                          uintf parent, uintf move_choice) {
...
Node *Trainer::get_node(uintf id) const { return hash_table[id]; }
...
void Trainer::move_down(uintf root, uintf move_choice) {
  ...
  for (uintf i = 0; i < NUM_MOVES; ++i) {
    if (root_node->has_visited(i) && i != move_choice)
      nodes.push(find_next(root_node->get_seed(), i));
  }
...
void Trainer::rehash_if_full() {
  ...
  std::cerr << "active nodes " << active_nodes << " table size "
            << hash_table.size() << '\n';
...
uintf Trainer::generate() { return generator(); }
...
void Trainer::place(uintf pos, uintf seed, const Game &game, uintf depth,
  ...
  // Placement new
  hash_table[pos] =
      new (cur_block + cur_ind) Node(seed, game, depth, parent, move_choice);
  ++cur_ind;
}
...
void Trainer::rehash() {
  ...
  std::cerr << "Rehashing! Old size: " << hash_table.size()
            << " New size: " << new_size << '\n';
  ...
  for (uintf i = 0; i < games.size(); ++i) {
    for (uintf j = 0; j < 2; ++j) {
      uintf root = games[i]->get_root(j),
            root_seed = get_node(root)->get_seed();
      ...
      for (uintf k = 0; k < NUM_MOVES; ++k) {
        ...
        if (hash_table[root]->has_visited(k)) {
          nodes.push(
              std::make_pair(find_next(root_seed, k), hash(root_seed, k)));
        }
      }
    }
  }
  ...
  while (!nodes.empty()) {
    uintf cur = nodes.front().first, seed = nodes.front().second,
          pos = seed % new_size;
    Node *cur_node = get_node(cur);
    while (new_table[pos]) {
      pos = (pos + 1) % new_size;
    ...
    for (uintf i = 0; i < NUM_MOVES; ++i) {
      if (cur_node->has_visited(i))
        nodes.push(std::make_pair(find_next(seed, i), hash(seed, i)));
    }
    nodes.pop();
  }
