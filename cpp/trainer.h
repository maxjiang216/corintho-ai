...
class Trainer {
  ...
  // We don't want children to be close to avoid clustering, but they do not
  // need to be too far Average capacity <LOAD_FACTOR should probabilistically
  // prevent long clusters Largest prime less than HASH_TABLE_SIZE / NUM_MOVES,
  // so that we never wrap around Guarantees no collision amongst siblings as
  // long as probe length is sufficiently short
  ...
  // Place node
  void place(uintf pos, uintf seed, const Game &game, uintf depth, uintf parent,
             uintf move_choice);
...
public:
  ...
  // This is used when receiving an opponent move the player did not search
  // (rare)
  uintf place_root(const Game &game, uintf depth);

  // Place child node in hash table
  uintf place_next(uintf seed, const Game &game, uintf depth, uintf parent,
                   uintf move_choice);
  // Find the child node
