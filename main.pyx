...
def do():
        cdef Trainer* x = new Trainer(3, 1, 200, 1, 0.25, "test", 0)
        cdef float[::1] evaluations = np.zeros(1, dtype=np.float32)
        cdef float[:, ::1] probabilities = np.zeros((3, 96), dtype=np.float32)
        cdef float[:, ::1] dirichlet = np.zeros((3, 96), dtype=np.float32)
        cdef float[:, ::1] game_states = np.zeros((3, 70), dtype=np.float32)
        x.do_iteration(&evaluations[0], &probabilities[0][0], &dirichlet[0][0], &game_states[0][0])
