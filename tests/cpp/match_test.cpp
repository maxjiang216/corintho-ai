...
TEST(MatchTest, Constructor) {
  TrainMCParams player1{1600, 16, 1.0, 0.25};
  TrainMCParams player2{1600, 16, 1.0, 0.25};
...
TEST(MatchTest, ConstructorRandom) {
  TrainMCParams player1{1600, 16, 1.0, 0.25, true};
  TrainMCParams player2{1600, 16, 1.0, 0.25};
  std::unique_ptr<std::ofstream> log_file = nullptr;
  Match match{0, player1, player2, std::move(log_file)};
  EXPECT_EQ(match.to_play(), 0);
  ...
  for (int32_t max_searches = 2; max_searches <= 16; ++max_searches) {
    for (int32_t searches_per_eval = 1; searches_per_eval <= max_searches;
      ...
      for (int32_t random_id = 0; random_id < 3; ++random_id) {
        TrainMCParams player1{max_searches, searches_per_eval, 1.0, 0.25,
                              random_id == 0};
        TrainMCParams player2{max_searches, searches_per_eval, 1.0, 0.25,
                              random_id == 1};
        std::unique_ptr<std::ofstream> log_file = nullptr;
