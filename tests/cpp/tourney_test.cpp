...
TEST(TourneyTest, FewSearches) {
  ...
  float game_states[4 * kGameStateSize * 12] = {0.0};
  float eval[4 * kGameStateSize * 12] = {0.0};
  float probs[4 * kNumMoves * 12] = {0.0};
...
TEST(TourneyTest, FullGame) {
  ...
  float game_states[16 * kGameStateSize * 8] = {0.0};
  float eval[16 * kGameStateSize * 8] = {0.0};
  float probs[16 * kNumMoves * 8] = {0.0};
  std::mt19937 generator(12345);
  std::uniform_real_distribution<float> prob_dist(0.0, 1.0);
  std::uniform_real_distribution<float> eval_dist(-1.0, 1.0);
  while (true) {
